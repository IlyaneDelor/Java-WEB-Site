 ...
 * @author user
 */
@WebServlet(urlPatterns = {"/serviceAccepted"})
public class serviceAccepted extends HttpServlet {
    Connection dbcon;
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            dbcon = DriverManager.getConnection(dbUrl, user, password);
              ...
              int test = (Integer) session1.getAttribute("id");
              PreparedStatement reqtest = dbcon.prepareStatement("SELECT `idWorker`FROM WORKER WHERE PERSON_idPerson = ?");
              reqtest.setInt(1,test);
                
              ResultSet reqserv = reqtest.executeQuery();
                reqserv.next();
            Integer idWo= (Integer) reqserv.getInt("idWorker");
         
            String idInter = request.getParameter("idIntervention");
            
            PreparedStatement requpda = dbcon.prepareStatement("UPDATE `intervention` SET `WORKER_idWORKER` = "+idWo+""+" WHERE idIntervention = "+idInter+"");
