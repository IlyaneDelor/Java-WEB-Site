<%-- 
    Document   : historique
    Created on : 4 mai 2020, 20:47:55
    Author     : user
--%>
...
<!DOCTYPE html>
    ...
    <body>
                        ...
                        <%
                            ...
                            int testt = (Integer) session1.getAttribute("id");
                          
                             PreparedStatement reqtest = dbcon.prepareStatement("SELECT `idWorker`FROM WORKER WHERE PERSON_idPerson = ?");
                             reqtest.setInt(1,testt);
                             
                            ResultSet resulttest  = reqtest.executeQuery();
                            resulttest.next();
                            int test = (int) resulttest.getInt("idWorker");
                             
                            
                            
                            PreparedStatement reqconnexion = dbcon.prepareStatement("SELECT * FROM intervention WHERE WORKER_idWORKER ="+test+" ");
			  ...
			  <div class="cadre">
				...
				<p class="text-serv">au <% out.println(resultreq.getString("addressIntervention")); %></p>
                                ...
                                <%
                                    ...
                                    int testa = Integer.parseInt(resultreq.getString("etat"));
                                    if(testa == 0) {
                                     %>
                                     <h3 class="text-serv">Etat : En Attente</h3>
                                     <%
                                    }else if(testa == 1){
                                    %>
                                     <h3 class="text-serv">Etat : Confirmé</h3>
                                   <%
                                    }else if(testa == 2) {
                                    %>
                                    <h3 class="text-serv">Etat : Annulée</h3>
                                    <%
                                    }
                             ...
                             <form method="post" action="serviceStopped" >
                                 <input type="hidden" name="idIntervention" value="<%  out.println(Integer.parseInt(resultreq.getString("idIntervention"))); %>" </input>
                                <button  type="submit" class="service-button" style="text-decoration: none;"  >Annuler</button>
                             </form>
